...
public:
  ...
  JustAudioEventSink::JustAudioEventSink(
      ...
      const std::string& id,
      std::shared_ptr<MainThreadDispatcher> dispatcher) : dispatcher_(std::move(dispatcher)) {
    event_channel_ =
  ...
  void Success(const EncodableValue& event) {
    ...
    dispatcher_->Post([self, event]() {
      if (auto locked = self.lock()) {
        if (locked->sink) {
          locked->sink->Success(event);
        }
      }
    });
  }
  ...
  void Error(const std::string& error_code,
    ...
    auto self = weak_from_this();
    dispatcher_->Post([self, error_code, error_message]() {
      if (auto locked = self.lock()) {
        if (locked->sink) {
          locked->sink->Error(error_code, error_message);
        }
      }
    });
  }
...
public:
  AudioPlayer::AudioPlayer(
    ...
    event_sink_ = std::make_shared<JustAudioEventSink>(
      messenger,
      "com.ryanheise.just_audio.events." + idx,
      dispatcher
    );
    data_sink_ = std::make_shared<JustAudioEventSink>(
      messenger,
      "com.ryanheise.just_audio.data." + idx,
      dispatcher
    );
